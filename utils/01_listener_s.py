...
from audio_input import AudioStream, AudioConfig
import logging
...
def record_audio(output_path: str = None, duration: float = None, verbose: bool = True):
    ...
    for dev in devices:
        if TARGET_DEVICE in dev["name"]:
            ...
            break
    print(f"已选择设备: [{device_index}] {device_name}")
    # # 配置音频参数
    # config = AudioConfig(
    #     sample_rate=16000,
    #     frame_length=1280,
    #     device_index=device_index,
    #     silence_threshold=900.0,  # 静音阈值
    #     max_phrase_duration=6.0 if duration is None else duration + 5.0,  # 最大录制时长
    #     silence_duration=1.0,  # 静音持续时间（秒）
    #     wait_for_silence_timeout=2.0,
    #     skip_initial_frames=5,
    #     required_silent_frames=5,
    #     no_speech_timeout=10.0,  # 无语音超时
    ...
    config = AudioConfig(
        ...
        frame_length=1280,
        device_index=device_index,
        silence_threshold=2000.0,  # 静音阈值 #500 
        max_phrase_duration=6.0 if duration is None else duration + 5.0,  # 最大录制时长
        silence_duration=0.5,  # 静音持续时间（秒）
...
def record_with_duration(output_path: str, duration: float, verbose: bool = True):
    ...
    # 自动选择最佳设备
    # device_index = AudioStream.select_best_device(target_sample_rate=44100)
    # if device_index is not None and verbose:
    #     device_info = next((d for d in devices if d['index'] == device_index), None)
    #     if device_info:
    #         print(f"已选择设备: [{device_index}] {device_info['name']}")
